...
Public Class QueryDFrm

    Public dSQL As String
    Public TabPage As String

    Private Sub QueryDFrm_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        If GridView.Rows.Count <> 0 Then
            ...
            If TabPage = "TP2" Then
                ...
                btn3.HeaderText = "Tagihan"
                btn3.Text = "Buat Tagihan"
                btn3.Name = "CmdCetak"
                btn3.UseColumnTextForButtonValue = True
                GridView.Columns.Add(btn3)
            End If
        End If
    End Sub
    ...
    Private Sub GridView_CellClick(ByVal sender As Object, ByVal e As System.Windows.Forms.DataGridViewCellEventArgs) Handles GridView.CellClick
        ...
        If TabPage = "TP1" Or TabPage = "TP2" Then
            Dim BtnCetak As Integer = GridView.ColumnCount - 1
            Dim BtnHistory As Integer = GridView.ColumnCount - 2
            Dim BtnEdit As Integer = GridView.ColumnCount - 3
            Dim BtnQryD1 As Integer = GridView.ColumnCount - 4
            If e.ColumnIndex = BtnQryD1 Then
                ...
                QueryD1Frm.Show()
            ElseIf e.ColumnIndex = BtnCetak Then
                If GridView.Rows(e.RowIndex).Cells("CloseF").Value = True Then
                    MessageBox.Show("Tagihan Sudah ditutup", "RSA", MessageBoxButtons.OK, MessageBoxIcon.Information)
                    Exit Sub
                Else
                    FakturTagihFrm.No_Nota = tostr(GridView.Rows(e.RowIndex).Cells("Surat_Jalan").Value)
                    FakturTagihFrm.NextID = toval(GridView.Rows(e.RowIndex).Cells("ID").Value)
                    FakturTagihFrm.Show()
                End If
                
            ElseIf e.ColumnIndex = BtnHistory Then
                SQL = "Select * From Barang_Keluar_H Where 1 = 1 and OSurat_Jalan = '" & GridView.Rows(e.RowIndex).Cells(17).Value & "' order by ID asc"
                QueryHistoryFrm.dSQL = SQL
                QueryHistoryFrm.TabPage = TabPage
                QueryHistoryFrm.Show()
